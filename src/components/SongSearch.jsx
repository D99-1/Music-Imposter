    ...
    <div className="w-full max-w-2xl space-y-6 md:space-y-8 px-2 md:px-0">
      {!selectedSong ? (
        <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 max-h-[350px] md:max-h-[400px] overflow-y-auto pr-1 custom-scrollbar no-scrollbar">
            {results.map(song => (
              <button
                key={song.id}
                onClick={() => handleSelect(song)}
                className="flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-secondary/5 hover:bg-secondary/10 rounded-2xl border border-transparent hover:border-highlight/30 transition-all text-left group active:bg-secondary/10"
              >
        ...
        <div className="flex flex-col items-center space-y-6 md:space-y-8 animate-in zoom-in duration-500">
          <AudioPreview
            song={selectedSong}
            startTime={startTime}
          ...
          <div className="flex flex-col md:flex-row gap-3 w-full max-w-md">
            <button
              ...
              className="w-full md:flex-1 py-4 md:py-5 rounded-2xl border border-secondary/10 font-black text-xs uppercase tracking-widest hover:bg-secondary/5 transition-all active:scale-95 flex items-center justify-center gap-2"
            >
              <X size={16} /> Cancel
            </button>
            <button
              onClick={() => onSelect({ ...selectedSong, startTime })}
              className="w-full md:flex-[2] py-4 md:py-5 rounded-2xl bg-highlight font-black text-xs uppercase tracking-widest text-white shadow-2xl shadow-highlight/40 hover:scale-[1.02] active:scale-95 transition-all"
            >
              LOCK IN SELECTION
            </button>
          </div>
